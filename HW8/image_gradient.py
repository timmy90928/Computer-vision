...
lap = cv2.Laplacian(img_gray_median, cv2.CV_64F, ksize=3)  # also pass kernal size
...
sobelX = cv2.Sobel(img_gray_median, cv2.CV_64F, 1, 0, ksize=3)  # here 1 means x direction
sobelY = cv2.Sobel(img_gray_median, cv2.CV_64F, 0, 1, ksize=3)  # here 1 means y direction
sobelX = np.uint8(np.absolute(sobelX))
sobelY = np.uint8(np.absolute(sobelY))
# finally combine sobelX and sobelY togather
sobelcombine = cv2.bitwise_not(cv2.bitwise_or(sobelX, sobelY))
